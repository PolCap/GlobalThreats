# ---------------------------------------------------------------------------- #
# - FILE NAME:   SensiMultiEffects.R         
# - DATE:        17/03/2022
# - DESCRIPTION: Check the sensitivity of our method to quantify interactive effects
...
data_int_mu <- data_int %>% 
  ...
  drop_na(ID) %>% 
  mutate(effect=ifelse(upper<mu, "Antagonistic", 
                       ifelse(mu<lower,
                              "Synergy", "Additive")))
...
data_freq_mu <- rbind(climate_change,
...
data_freq_mu_taxon <- rbind(climate_change,
                         habitat_loss, exploitation, 
                         disease, invasive, 
                         pollution)
...
setwd(ResultPath)
save(data_int2, data_prop_mu, 
     data_freq_mu, data_freq_mu_taxon,
     file="SensiMEff.RData")
...
(row1 <- plot_grid(gs5a+ggtitle("Simple additive"), 
                   gs5b+ggtitle("Multiplicative"), 
                   gs5c+ggtitle("Dominance"),
                   gs5d+ggtitle("Simple additive"), 
                   gs5e+ggtitle("Multiplicative"), 
                   gs5f+ggtitle("Dominance"), 
                   nrow=2, labels="auto"))

# Get the legend

legend <- get_legend(gs5a+theme(legend.position = "bottom",
                                legend.text = element_text(size=14)))
# Plot it 

(figS5<- plot_grid(row1, legend, 
                   nrow = 2, rel_heights = c(1,0.05)))


# Save it

ggsave("Fig. S5.pdf", figS5, 
       width = 16, height = 14,
       path = ResultPath)
